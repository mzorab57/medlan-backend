...
class BrandController
{
    public function index(): void
    {
        ...
        $sql = "SELECT id, name, slug, image, is_active, created_at, updated_at FROM brands $where ORDER BY id LIMIT ? OFFSET ?";
        $types .= 'ii';
        $params[] = (int)$per;
        $params[] = (int)$offset;
    ...
    public function show($id): void
    {
        global $conn;
        $id = (int)$id;
        $stmt = $conn->prepare('SELECT id, name, slug, image, is_active, created_at, updated_at FROM brands WHERE id = ?');
    ...
    public function store(): void
    {
        ...
        $stmt = $conn->prepare('INSERT INTO brands (name, slug, image, is_active) VALUES (?, ?, ?, ?)');
        $stmt->bind_param('sssi', $name, $slug, $image, $active);
        if (!$stmt->execute()) {
            if ($conn->errno === 1062) {
    ...
    public function update($id): void
    {
        global $conn;
        $id = (int)$id;
        $check = $conn->prepare('SELECT id FROM brands WHERE id = ?');
    ...
    public function imageUpload($id): void
    {
        ...
        $destDir = __DIR__ . '/../' . rtrim($uploadBase, '/\\') . '/brands/';
        $newName = null;
        ...
        $absolute = $ok[2];
        $relative = str_replace(realpath(__DIR__ . '/..') . DIRECTORY_SEPARATOR, '', realpath($absolute)) ?: (rtrim($uploadBase, '/\\') . '/brands/' . basename($absolute));
        $st = $conn->prepare('UPDATE brands SET image = ? WHERE id = ?');
        $st->bind_param('si', $relative, $bid);
        $st->execute();
