...
class SubcategoryController
{
    public function index(): void
    {
        ...
        $sql = "SELECT id, category_id, name, slug, image, display_order, is_active, created_at, updated_at FROM subcategories $whereSql ORDER BY display_order, id LIMIT ? OFFSET ?";
        $types .= 'ii';
        $params[] = (int)$per;
        $params[] = (int)$offset;
    ...
    public function show($id): void
    {
        global $conn;
        $id = (int)$id;
        $stmt = $conn->prepare('SELECT id, category_id, name, slug, image, display_order, is_active, created_at, updated_at FROM subcategories WHERE id = ?');
    ...
    public function store(): void
    {
        ...
        if ($categoryId <= 0 || $name === '') {
            jsonResponse(false, 'category_id و name پێویستن', null, 422);
            return;
        }
        if ($slug === '') {
            $slug = generateSlug($name);
        }
        $stmt = $conn->prepare('INSERT INTO subcategories (category_id, name, slug, image, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->bind_param('isssii', $categoryId, $name, $slug, $image, $display, $active);
        if (!$stmt->execute()) {
            if ($conn->errno === 1062) {
    ...
    public function update($id): void
    {
        global $conn;
        $id = (int)$id;
        $check = $conn->prepare('SELECT id FROM subcategories WHERE id = ?');
    ...
    public function imageUpload($id): void
    {
        global $conn;
        $sid = (int)$id;
        $chk = $conn->prepare('SELECT id FROM subcategories WHERE id = ?');
        ...
        $destDir = __DIR__ . '/../' . rtrim($uploadBase, '/\\') . '/subcategories/';
        $newName = null;
        ...
        $absolute = $ok[2];
        $relative = str_replace(realpath(__DIR__ . '/..') . DIRECTORY_SEPARATOR, '', realpath($absolute)) ?: (rtrim($uploadBase, '/\\') . '/subcategories/' . basename($absolute));
        $st = $conn->prepare('UPDATE subcategories SET image = ? WHERE id = ?');
